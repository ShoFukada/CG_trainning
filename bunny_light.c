...
model bunny;
const char *file_name = "bunny_light.obj";
double aspect;
...
void display() {
    ...
    glBegin(GL_TRIANGLES);
        for(int i=0;i < bunny.num_vertices/3;i++) {
            if(i%3==0) {
                GLfloat normal[3] = {bunny.normals[i],bunny.normals[i+1],bunny.normals[i+2]};
                glNormal3fv(normal);
            }
...
int main(int argc, char** argv) {
    ...
    // FILE *fp;
    // fp = fopen("v_test.txt", "w");
    // for(int i=0;i<bunny.num_vertices/3;i++) {
    //     fprintf(fp, "v %f %f %f\n",bunny.vertices[3*i],bunny.vertices[3*i+1],bunny.vertices[3*i+2]);
    // }
    // fclose(fp);
